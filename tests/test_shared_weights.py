...
def test_mindspore_loading(weights_path):
    ...
    try:
        ...
        from precision_compare.model.llama_ms import create_ms_model as create_ms_model
        from precision_compare.model.llama_ms import load_model_with_weights
    except ImportError:
...
def check_weights_compatibility(weights_path):
    ...
    try:
        import torch
        import mindspore as ms
        from precision_compare.model.model import (
            create_test_model as create_torch_model,
        )
        from precision_compare.model.llama_ms import create_ms_model as create_ms_model
    except ImportError:
    ...
    # 进行兼容性检查
    result = WeightManager.check_weights_compatibility(
        torch_model, ms_model, weights_path
    )
    ...
    print(
        f"MindSpore模型可加载参数比例: {result['mindspore_check']['loadable_percent']}"
    )
    ...
    if result["torch_check"]["missing_params"] > 0:
        ...
        if result["torch_check"]["missing_details"]:
            print(
                f"部分缺失参数: {', '.join(result['torch_check']['missing_details'][:5])}"
            )

    if result["mindspore_check"]["missing_params"] > 0:
        print(
            f"MindSpore模型缺失参数数量: {result['mindspore_check']['missing_params']}"
        )
        if result["mindspore_check"]["missing_details"]:
            print(
                f"部分缺失参数: {', '.join(result['mindspore_check']['missing_details'][:5])}"
            )

    # 保存报告到文件
    with open(
        weights_path.replace(".safetensors", "_compatibility.json"),
        "w",
        encoding="utf-8",
    ) as f:
    ...
    print(
        f"兼容性报告已保存到: {weights_path.replace('.safetensors', '_compatibility.json')}"
    )
