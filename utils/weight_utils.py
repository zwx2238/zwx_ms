...
class WeightManager:
    ...
    """
    @staticmethod
    def generate_shared_weights(model_config: Dict[str, Any], output_path: str, seed: int = 42) -> str:
        """
        ...
        # 嵌入层权重
        weights["model.embed_tokens.weight"] = torch.normal(0, 0.02, (vocab_size, hidden_size))
        ...
        for i in range(num_hidden_layers):
            ...
            weights[attn_prefix + "q_proj.weight"] = torch.normal(0, 0.02, (hidden_size, hidden_size))
            weights[attn_prefix + "k_proj.weight"] = torch.normal(0, 0.02, (hidden_size, hidden_size))
            weights[attn_prefix + "v_proj.weight"] = torch.normal(0, 0.02, (hidden_size, hidden_size))
            weights[attn_prefix + "o_proj.weight"] = torch.normal(0, 0.02, (hidden_size, hidden_size))
            ...
            weights[layer_prefix + "post_attention_layernorm.weight"] = torch.ones(hidden_size)
            weights[layer_prefix + "post_attention_layernorm.bias"] = torch.zeros(hidden_size)
            
            # MLP层权重
            weights[layer_prefix + "mlp.gate_proj.weight"] = torch.normal(0, 0.02, (intermediate_size, hidden_size))
            weights[layer_prefix + "mlp.up_proj.weight"] = torch.normal(0, 0.02, (intermediate_size, hidden_size))
            weights[layer_prefix + "mlp.down_proj.weight"] = torch.normal(0, 0.02, (hidden_size, intermediate_size))
        ...
        with open(output_path.replace(".safetensors", "_metadata.json"), "w", encoding="utf-8") as f:
    ...
    def load_torch_weights(model, weights_path: str, strict: bool = True) -> List[str]:
        ...
        if os.path.exists(metadata_path):
            try:
                ...
                print(f"已加载权重元数据, 模型配置包含 {len(metadata.get('model_config', {}))} 项")
        ...
        for name, param in model_state.items():
            if name in weights:
                ...
                if param.shape == weights[name].shape:
                    ...
                    print(f"警告: 参数 {name} 形状不匹配 - 模型: {param.shape} vs 权重: {weights[name].shape}")
                ...
                for weight_name in weights:
                    if weight_name.endswith(name.split(".")[-1]) and param.shape == weights[weight_name].shape:
        ...
        if missing_keys and strict:
            ...
            print(f"未加载的参数: {missing_keys[:5]}{'...' if len(missing_keys) > 5 else ''}")
        
        return missing_keys
    
    @staticmethod
    def load_mindspore_weights(model, weights_path: str, strict: bool = True) -> List[str]:
        """
        为MindSpore模型加载权重
                ...
                for torch_name, torch_weight in weights.items():
                    ...
                    for i in range(len(torch_parts)):
                        ...
                        if i > 0:  # 跳过第一层，通常是模型名
                            ...
                            # 检查部分路径映射
                            if partial_torch_path in torch_to_ms_map and torch_to_ms_map[partial_torch_path] == partial_ms_path:
                                mapped_torch_name.append(torch_part)
                                continue
                        ...
                        if i == len(torch_parts) - 1:  # 最后一部分是参数名
                            ...
                            if last_two_torch in torch_to_ms_map and torch_to_ms_map[last_two_torch] == last_two_ms:
                                mapped_torch_name.append(torch_part)
                                continue
            ...
            if not found:
                for torch_name, torch_weight in weights.items():
                    ...
                    if ms_param.shape == torch_weight.shape:
                        ...
                        # 检查特殊映射关系
                        if (ms_last == "embedding_table" and torch_last == "weight") or \
                           (ms_last == "gamma" and torch_last == "weight") or \
    ...
    @staticmethod
    def check_weights_compatibility(torch_model, ms_model, weights_path: str) -> Dict[str, Any]:
        """
        检查权重与两个框架模型的兼容性
