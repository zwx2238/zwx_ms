...
class AnalysisReport:
    ...
    def save_diff_table(self, diffs: List[TensorDiff]):
        ...
        for diff in diffs:
            table_data.append([
                ...
                f"{diff.mean_abs_diff:.2e}",
                f"{diff.relative_diff:.2%}",
                str(diff.location),
                str(diff.shape)
            ])
        
        headers = ["模块", "类型", "最大绝对差异", "平均绝对差异", "相对差异", "最大差异位置", "张量形状"]
    ...
    def plot_diff_heatmap(self, diffs: List[TensorDiff]):
        ...
        if len(diffs) < 2:
            ...
            plt.bar(['最大差异', '平均差异', '相对差异'], 
                   [diffs[0].max_abs_diff, diffs[0].mean_abs_diff, diffs[0].relative_diff],
                   color='red')
        ...
        plt.figure(figsize=(15, max(10, module_count * 0.5)))
        ...
        for diff in diffs:
            # 保留完整的模块名，不进行截断
            module_names.append(diff.module_name)
            max_diffs.append(float(diff.max_abs_diff))
            mean_diffs.append(float(diff.mean_abs_diff))
            rel_diffs.append(float(diff.relative_diff))
        
        # 创建热力图数据
        data = np.array([max_diffs, mean_diffs, rel_diffs]).T
        
        # 创建DataFrame
        df = pd.DataFrame(
            data,
            index=module_names,
            columns=['最大绝对差异', '平均绝对差异', '相对差异']
        )
    ...
    def plot_diff_distribution(self, diffs: List[TensorDiff]):
        ...
        if len(diffs) < 3:
            ...
            for diff in diffs:
                ...
                values.append(diff.relative_diff)
            
            # 绘制条形图
            plt.bar(range(len(labels)), values, color='orange', width=0.6)
            plt.xticks(range(len(labels)), labels, rotation=45, ha='right', fontsize=10)
            plt.title('模块相对差异', fontsize=14)
            plt.ylabel('相对差异', fontsize=12)
            plt.subplots_adjust(bottom=0.3)  # 为x轴标签留出更多空间
            plt.tight_layout()
        ...
        # 准备数据，确保所有值都是float类型
        max_diffs = [float(d.max_abs_diff) for d in diffs]
        mean_diffs = [float(d.mean_abs_diff) for d in diffs]
        rel_diffs = [float(d.relative_diff) for d in diffs]
        
        # 创建三个子图
        plt.subplot(131)
        plt.hist(max_diffs, bins=min(20, len(diffs)), alpha=0.7, color='skyblue')
        ...
        plt.subplot(132)
        plt.hist(mean_diffs, bins=min(20, len(diffs)), alpha=0.7, color='lightgreen')
        plt.title('平均绝对差异分布', fontsize=14)
        plt.xlabel('差异值', fontsize=12)
        plt.ylabel('频次', fontsize=12)
        plt.grid(axis='y', alpha=0.3)
        
        plt.subplot(133)
        plt.hist(rel_diffs, bins=min(20, len(diffs)), alpha=0.7, color='salmon')
        plt.title('相对差异分布', fontsize=14)
        plt.xlabel('差异值', fontsize=12)
        plt.ylabel('频次', fontsize=12)
        plt.grid(axis='y', alpha=0.3)
    ...
    def save_summary(self, diffs: List[TensorDiff]):
        ...
        summary = {
            "总体统计": {
                "分析的模块数": len(diffs),
                "最大相对差异": float(max(d.relative_diff for d in diffs)),
                "平均相对差异": float(sum(d.relative_diff for d in diffs) / len(diffs)),
                "最大绝对差异": float(max(d.max_abs_diff for d in diffs)),
                "平均绝对差异": float(sum(d.mean_abs_diff for d in diffs) / len(diffs))
            },
            "按执行顺序的显著差异模块": [
                {
                    "模块名": d.module_name,
                    "类型": d.tensor_type,
                    "相对差异": f"{float(d.relative_diff):.2%}"
                }
                for d in diffs if d.relative_diff > 0.01
            ][:5]  # 只显示前5个显著差异
        }
    ...
    def plot_diff_propagation(self, diffs: List[TensorDiff]):
        """绘制差异传播路径图"""
        if not diffs:
            # 创建一个空图，显示"无差异"信息
            plt.figure(figsize=(8, 6))
            plt.text(0.5, 0.5, "无显著差异", 
                     ...
                     fontsize=20)
            plt.axis('off')
            plt.savefig(self.output_dir / f"diff_propagation_{self.timestamp}.png", dpi=300, bbox_inches='tight')
            plt.close()
            return
        ...
        if len(diffs) < 2:
            ...
            plt.text(0.5, 0.5, f"只有一个数据点: {diffs[0].module_name}\n相对差异: {diffs[0].relative_diff:.2e}", 
                     horizontalalignment='center', 
                     verticalalignment='center',
                     fontsize=16)
            plt.axis('off')
        ...
        # 提取有意义的差异
        significant_diffs = [d for d in diffs if d.relative_diff > 0.001]
        if not significant_diffs:
            plt.figure(figsize=(8, 6))
            plt.text(0.5, 0.5, "没有显著差异(>0.1%)", 
                     horizontalalignment='center', 
                     verticalalignment='center',
                     fontsize=20)
            plt.axis('off')
        ...
        # 创建模块执行顺序的有向图
        # 根据模块数量调整图形大小
        num_modules = len(significant_diffs)
        plt.figure(figsize=(max(15, num_modules * 1.2), max(12, num_modules * 0.8)))
        ...
        for diff in significant_diffs:
            # 保留完整模块名，不再简化
            # 根据tensor类型添加后缀
            labeled_name = f"{diff.module_name} ({diff.tensor_type})"
            modules.append(labeled_name)
            rel_diffs.append(diff.relative_diff)
        ...
        if num_modules == 2:
            ...
            # 圆形布局，但增加半径以适应更长的标签
            r = max(0.8, num_modules * 0.1)  # 根据模块数量动态调整半径
        ...
        ax = plt.gca()
        scatter = ax.scatter(
            [pos[i][0] for i in range(num_modules)],
            [pos[i][1] for i in range(num_modules)],
            s=400,  # 增大节点大小
            c=[rel_diffs[i] for i in range(num_modules)],
            cmap=cmap,
            zorder=10
        )
        ...
        for i, module in enumerate(modules):
            # 增加文本大小并使用更宽的文本框
            bbox_props = dict(boxstyle="round,pad=0.5", fc="white", ec="gray", alpha=0.9, linewidth=1.5)
            plt.annotate(
                module,
                xy=pos[i],
                xytext=(pos[i][0]*1.3, pos[i][1]*1.3),  # 将标签放置更远
                fontsize=9,  # 增大字体
                ha='center',
                va='center',
                bbox=bbox_props,
        ...
        cbar.set_label('相对差异', fontsize=12)
        
        plt.title('差异传播路径', fontsize=16)
        plt.axis('off')
        plt.tight_layout()
        
        # 保存图片，增加DPI确保高清晰度
        plt.savefig(self.output_dir / f"diff_propagation_{self.timestamp}.png", dpi=400, bbox_inches='tight')
        plt.close()
...
class ModelAnalyzer:
    ...
    def compare_tensors(self, t1: torch.Tensor, t2: torch.Tensor) -> Tuple[float, float, float, float, Tuple[int, ...]]:
        """比较两个张量的差异
        ...
        # 相对差异 (除以最大值以避免除零)
        denominator = torch.maximum(torch.abs(t1), torch.abs(t2))
        # 添加小常数避免除零
        denominator = torch.where(denominator > 0, denominator, torch.tensor(1e-10, device=denominator.device))
        rel_diff = torch.mean(abs_diff / denominator).item()
        ...
        return max_diff, mean_diff, rel_diff, cosine, location
    ...
    def compare_module_data(self, module_path: str) -> List[TensorDiff]:
                ...
                if isinstance(base_inputs, (list, tuple)):
                    for idx, (base_item, ref_item) in enumerate(zip(base_inputs, ref_inputs)):
                        if isinstance(base_item, torch.Tensor) and isinstance(ref_item, torch.Tensor):
                            max_diff, mean_diff, rel_diff, cosine, loc = self.compare_tensors(base_item, ref_item)
                    ...
                    max_diff, mean_diff, rel_diff, cosine, loc = self.compare_tensors(base_inputs, ref_inputs)
                    diffs.append(TensorDiff(
                        module_path, 
                        "inputs", 
                        max_diff, 
        ...
        if base_output_file and ref_output_file:
            try:
                ...
                if isinstance(base_outputs, (list, tuple)):
                    ...
                    max_diff, mean_diff, rel_diff, cosine, loc = self.compare_tensors(base_outputs, ref_outputs)
                    diffs.append(TensorDiff(
                        module_path, 
                        "outputs", 
                        max_diff, 
        ...
        for param_file in base_param_files:
            ...
            try:
                ...
                max_diff, mean_diff, rel_diff, cosine, loc = self.compare_tensors(base_param, ref_param)
                diffs.append(TensorDiff(
    ...
    def analyze_all(self, threshold: float = 1e-5) -> List[TensorDiff]:
        """分析所有模块的差异"""
        all_diffs = []
        ...
        # 使用执行顺序信息排序
        all_diffs.sort(key=lambda x: self.get_execution_index(x.module_name, x.tensor_type))
    ...
    def find_problematic_ops(self) -> List[str]:
        ...
        # 按照执行顺序排序的差异列表
        significant_diffs = [d for d in diffs if d.relative_diff > 0.01]  # 使用1%的相对差异作为阈值
        ...
        # 2. 找到差异最大的模块
        max_diff = max(significant_diffs, key=lambda d: d.relative_diff)
        if max_diff.module_name != first_diff.module_name:
            problematic_ops.append(f"{max_diff.module_name} (最大相对差异: {max_diff.relative_diff:.2%})")
        ...
        for module, module_diff_list in module_diffs.items():
            ...
            for i in range(1, len(sorted_diffs)):
                ...
                if (curr.tensor_type == "outputs" and 
                    ...
                    curr.relative_diff < prev.relative_diff * 0.5):
...
def analyze_runs(base_dir: str, ref_dir: str, output_dir: str = None) -> Dict[str, Any]:
    ...
    # 绘制差异传播图
    report.plot_diff_propagation(diffs)
    ...
    for diff in diffs:
        if diff.tensor_type == "outputs":
            result["module_comparisons"][diff.module_name] = {
                ...
                "relative_diff": diff.relative_diff,
